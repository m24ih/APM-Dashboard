      ...
      >
        ...
        {payload.map((p, i) => (
          <p key={i} style={{ color: p.color, margin: "2px 0" }}>
            {p.name}:{" "}
            {p.value === -1 ? (
              <span style={{ color: "#f59e0b" }}>iz miktari (trace)</span>
            ) : (
              `${p.value} mm`
            )}
          </p>
    ...
    <div style={{ display: "grid", gap: 24 }}>
      <div style={{ background: "#1e293b", padding: 24, borderRadius: 12 }}>
        <h2 style={{ color: "#e2e8f0", marginBottom: 8 }}>
          Yagis Miktari (Gercek Zamanli)
        </h2>
        <p style={{ color: "#94a3b8", fontSize: 12, marginBottom: 16 }}>
          <span style={{ color: "#f59e0b" }}>●</span> Sari isaretler iz
          miktarini (trace, -1) temsil eder
        </p>
        {data.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={350}>
            <ComposedChart data={data}>
              ...
              <Line
                type="monotone"
                dataKey="precip1HR_display"
                stroke="#38bdf8"
                dot={<CustomDot dataKey="precip1HR" />}
                name="1 Saatlik Yagis (mm)"
                strokeWidth={2}
              />
              <Line
                type="monotone"
                dataKey="precip6HR_display"
                stroke="#a78bfa"
                dot={<CustomDot dataKey="precip6HR" />}
                name="6 Saatlik Yagis (mm)"
                strokeWidth={2}
              />
            </ComposedChart>
          </ResponsiveContainer>
